...
### Iteration 31: 扩展文件工具集

**目标**: 添加更多实用文件工具，完善 AI Agent 文件管理能力

新增工具:
- [x] `tree` - 显示目录树结构
  - 支持 path, depth, show_hidden 参数
  - 递归遍历目录，支持深度限制
  - 美观的树形输出格式
...
- [x] `hash` - 计算文件哈希
  - 支持 sha256, sha512, sha1, md5 算法
  - 默认使用 sha256
  - 输出格式兼容 unix md5sum/sha256sum

- [x] `wc` - 统计文件行/词/字符数
  - 支持 lines, words, bytes, chars 参数
  - 默认显示行数和字节数 (兼容 unix wc)
  - 灵活的输出控制
...
测试增强:
- [x] 新增 16 个测试用例 (tree × 4, chmod × 3, hash × 5, wc × 4)

代码质量:
- [x] cargo clippy - 通过 (0 警告)
- [x] cargo test - 154 个测试通过 (+16 新测试)
...
- [x] `tui/` 模块
  - `TuiApp` - TUI 应用主类
  - `AppState` - 应用状态管理
  - `components.rs` - UI 组件 (会话列表、消息面板、输入框、帮助栏)
  - `app.rs` - 事件处理与渲染
